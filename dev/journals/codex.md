...
Implemented a new opt-in `declared` attachment mode alongside the existing legacy `shared-root` path. The new mode still uses one VM-level shared-root virtiofs export plus stable host-side staged bind targets, but it now writes a host-synced manifest for declared attachments, installs a guest-side systemd replay helper, and uses that helper to restore or repair guest-visible bind mounts from persisted intent instead of rebuilding the whole bind graph on every `aivm code .` / `aivm ssh .` style entry. I wired that through the attachment resolver, attach/detach flows, session reconcile/restore logic, docs, and focused regression tests without removing or changing the old shared-root behavior.
...
Uncertainties/risks: the guest replay helper is intentionally conservative and shell-command-driven, so it still depends on common guest tools like `mount`, `findmnt`, and `mountpoint`, and it currently rewrites/refreshes the guest replay helper each time the declared path reconciles instead of doing a tighter diff-based install. I also chose not to retire the old `shared-root` mode yet, so there is some temporary duplication between the legacy direct guest-bind path and the new declared replay path. Another tradeoff is that boot-time replay focuses on guest-visible bind mounts, not the richer companion-symlink/mirror-home behavior for every secondary attachment; interactive reconcile still applies the higher-level symlink behavior for the active attachment.
...
What I am confident about: the new declared mode is implemented as a separate backend, default mode selection remains legacy `shared-root` for backwards compatibility, explicit attach/detach/code/ssh flows understand the new mode, focused tests cover manifest persistence, replay orchestration, read-only vs read-write declarations, reconcile path integration, and detach behavior, and the targeted regression runs are green (`51 passed` across declared/resolve/session/detach tests plus `43 passed` across shared-root/guest/update compatibility tests).
