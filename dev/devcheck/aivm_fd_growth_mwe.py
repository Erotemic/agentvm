...
def default_managed_host_export_dir(vm_name: str) -> Path:
    return (
        DEFAULT_MANAGED_SHARED_ROOT_BASE
        / sanitize_tag_component(vm_name)
        / 'shared-root'
    )
...
def default_virtiofs_tag(args: argparse.Namespace) -> str:
    ...
    return (
        DEFAULT_MANAGED_VIRTIOFS_TAG
        if policy == 'managed'
        else DEFAULT_REUSE_VIRTIOFS_TAG
    )
...
def run(argv: Sequence[str], check: bool = False) -> CmdResult:
    ...
    res = CmdResult(
        list(argv), proc.returncode, proc.stdout or '', proc.stderr or ''
    )
...
def host_run(
    args: argparse.Namespace, argv: Sequence[str], check: bool = False
) -> CmdResult:
...
def host_findmnt(args: argparse.Namespace, path: str) -> CmdResult:
    return host_run(
        args,
        ['findmnt', '-M', path, '-o', 'TARGET,SOURCE,FSTYPE,OPTIONS', '-n'],
    )


def guest_findmnt(args: argparse.Namespace, path: str) -> CmdResult:
    return guest_run(
        args, f'findmnt -M {q(path)} -o TARGET,SOURCE,FSTYPE,OPTIONS -n'
    )
...
def guest_path_exists(
    args: argparse.Namespace, path: str, use_sudo: bool = False
) -> bool:
...
def ensure_vm_share_mapping(
    args: argparse.Namespace, state: dict[str, Any], modify: bool
) -> None:
    ...
    if exact:
        return
    conflicting_tag = [
        (src, tgt) for src, tgt in current if tgt == tag and src != source_dir
    ]
    conflicting_src = [
        (src, tgt) for src, tgt in current if src == source_dir and tgt != tag
    ]
...
def ensure_guest_shared_root_mount(
    args: argparse.Namespace, state: dict[str, Any], modify: bool
) -> None:
    tag = state['env']['virtiofs_tag']
...
def ensure_shared_root_environment(
    args: argparse.Namespace, state: dict[str, Any], modify: bool
) -> None:
    policy = state['env']['shared_root_policy']
...
def wait_for_vm_state(
    args: argparse.Namespace, want_running: bool, timeout_s: float
) -> None:
    ...
    raise RuntimeError(
        f'timed out waiting for VM {args.vm_name} to become {want}; current state={state!r}'
    )
...
def wait_for_ssh_ready(args: argparse.Namespace, timeout_s: float) -> None:
    ...
    probe = [
        ...
        'BatchMode=yes',
        '-o',
        f'ConnectTimeout={int(max(1, min(timeout_s, 5)))}',
    ]
    if args.ssh_identity:
    ...
    raise RuntimeError(
        f'timed out waiting for SSH readiness on {args.ssh_target!r}{detail}'
    )
...
def shutdown_vm(args: argparse.Namespace) -> None:
    ...
    try:
        wait_for_vm_state(
            args, want_running=False, timeout_s=args.vm_shutdown_timeout
        )
        return
    except RuntimeError:
        if not args.force_destroy_on_shutdown_timeout:
            raise
    show(
        'virsh destroy (shutdown timeout fallback)',
        host_run(args, ['virsh', 'destroy', args.vm_name]),
    )
    wait_for_vm_state(
        args,
        want_running=False,
        timeout_s=max(10.0, args.vm_shutdown_timeout / 2.0),
    )
...
def restart_vm_into_measurement_env(
    ...
    if policy == 'managed':
        host_run(
            args, ['mkdir', '-p', state['env']['host_export_dir']], check=True
        )
    shutdown_vm(args)
...
def fd_count_for_pid(args: argparse.Namespace, pid: str) -> int | None:
    res = host_run(
        ...
        [
            'bash',
            '-lc',
            f'test -d /proc/{q(pid)}/fd && ls /proc/{q(pid)}/fd | wc -l || true',
        ],
    )
...
def fd_entries_for_pid(
    args: argparse.Namespace, pid: str
) -> list[dict[str, str]]:
    code = """
import json
import os
...
def discover_relevant_holders(
    ...
    probe_paths = [state['paths']['host_stage_root']] + [
        slot['host_stage'] for slot in state['slots']
    ]
...
def scoped_fd_metrics(
    ...
    slot_stage_fd_totals: dict[str, int] = {
        slot['name']: 0 for slot in state['slots']
    }
...
def relevant_holder_snapshot(
    ...
    entries_by_pid = {
        pid: fd_entries_for_pid(args, pid) for pid in sorted(relevant)
    }
    fd_counts = {
        pid: len(entries_by_pid.get(pid, []))
        if entries_by_pid.get(pid) is not None
        else None
    ...
    lsof_stage_root_entries = lsof_entries_under_path(
        args, state['paths']['host_stage_root']
    )
    lsof_scratch_root_entries = lsof_entries_under_path(
        args, state['paths']['host_scratch_root']
    )
    ...
    lsof_stage_root = summarize_lsof_entries(
        lsof_stage_root_entries, relevant_pids
    )
    lsof_scratch_root = summarize_lsof_entries(
        lsof_scratch_root_entries, relevant_pids
    )
    return {
        'label': label,
        'probe_paths': [state['paths']['host_stage_root']]
        ...
        'lsof_run_total_count': lsof_stage_root['record_count']
        + lsof_scratch_root['record_count'],
        'lsof_run_relevant_total_count': (
            lsof_stage_root['relevant_record_count']
            + lsof_scratch_root['relevant_record_count']
        ),
...
def current_mount_state(
    ...
    return {
        ...
        'num_host_stage_mounted': sum(
            1 for mounted in host_stage_mounted.values() if mounted
        ),
        'num_guest_dst_mounted': sum(
            1 for mounted in guest_dst_mounted.values() if mounted
        ),
    }


def lsof_entries_under_path(
...
def collect_dirty_state(
    ...
    dirty: dict[str, Any] = {
        'host_stage_root_exists': host_path_exists(
            args, state['paths']['host_stage_root']
        ),
        'host_scratch_root_exists': host_path_exists(
            args, state['paths']['host_scratch_root']
        ),
        'guest_dst_root_exists': guest_path_exists(
            args, state['paths']['guest_dst_root'], use_sudo=True
        ),
...
def derive_state(args: argparse.Namespace) -> dict[str, Any]:
    ...
    policy = getattr(args, 'shared_root_policy', 'managed')
    guest_shared_base = str(
        getattr(args, 'guest_shared_base', None) or DEFAULT_GUEST_SHARED_BASE
    )
    ...
    if getattr(args, 'host_export_dir', None):
        ...
        matches = [
            src for src, tgt in vm_share_mappings(args) if tgt == virtiofs_tag
        ]
        if len(matches) == 1:
...
def preflight(args: argparse.Namespace, state: dict[str, Any]) -> None:
    ...
    show_if_any(
        ...
        guest_run(
            args,
            f'findmnt -T {q(state["paths"]["guest_stage_root"])} || findmnt -T {q(state["env"]["guest_shared_base"])}',
        ),
    )
    show_if_any(
        ...
        host_run(
            ...
            [
                'bash',
                '-lc',
                f'test -d {q(state["env"]["host_export_dir"])} && echo ok || (echo missing; exit 1)',
            ],
        ),
    )
    show_if_any(
        ...
        host_run(
            ...
            [
                'bash',
                '-lc',
                'command -v lsof >/dev/null && echo ok || (echo missing; exit 1)',
            ],
        ),
    )
    if state['slots']:
    ...
    should_fail = getattr(args, 'fail_dirty_preflight', False) and args.cmd in {
        'setup',
        'loop',
    }
...
def prepare_host_sources(
    ...
    host_run(
        args,
        ['mkdir', '-p', state['paths']['host_guest_echo_root']],
        check=True,
    )
    for slot in state['slots']:
        src = slot['host_src']
        ...
        """
        show(
            'prepare source ' + slot['name'],
            host_run(args, ['bash', '-lc', script]),
        )
...
def ensure_host_stages(args: argparse.Namespace, state: dict[str, Any]) -> None:
    ...
    host_run(
        args, ['mkdir', '-p', state['paths']['host_stage_root']], check=True
    )
    for slot in state['slots']:
        host_run(args, ['mkdir', '-p', slot['host_stage']], check=True)
        if host_is_exact_mountpoint(args, slot['host_stage']):
            show_if_any(
                f'findmnt host stage {slot["name"]} (already mounted)',
                host_findmnt(args, slot['host_stage']),
            )
            continue
        show(
            f'mount --bind host_src -> host_stage ({slot["name"]})',
            host_run(
                args, ['mount', '--bind', slot['host_src'], slot['host_stage']]
            ),
        )
        show_if_any(
            f'findmnt host stage {slot["name"]}',
            host_findmnt(args, slot['host_stage']),
        )


def detach_host_stages(
    args: argparse.Namespace, state: dict[str, Any], lazy: bool = False
) -> None:
    ...
    for slot in state['slots']:
        ...
        show_if_any(
            f'findmnt host stage {slot["name"]} (after)',
            host_findmnt(args, slot['host_stage']),
        )


def attach_guest_exposures(
    args: argparse.Namespace, state: dict[str, Any]
) -> None:
    banner('ATTACH GUEST EXPOSURES')
    guest_run(
        args,
        f'mkdir -p {q(state["paths"]["guest_dst_root"])}',
        use_sudo=True,
        check=True,
    )
    for slot in state['slots']:
        guest_run(
            args, f'mkdir -p {q(slot["guest_dst"])}', use_sudo=True, check=True
        )
        if guest_is_exact_mountpoint(args, slot['guest_dst']):
            show_if_any(
                f'findmnt guest dst {slot["name"]} (already mounted)',
                guest_findmnt(args, slot['guest_dst']),
            )
            continue
        show(
            ...
            guest_run(
                args,
                f'mount --bind {q(slot["guest_stage"])} {q(slot["guest_dst"])}',
                use_sudo=True,
            ),
        )
        show_if_any(
            f'findmnt guest dst {slot["name"]}',
            guest_findmnt(args, slot['guest_dst']),
        )


def detach_guest_exposures(
    args: argparse.Namespace, state: dict[str, Any], lazy: bool = False
) -> None:
    ...
    for slot in state['slots']:
        ...
        show(
            ...
            guest_run(
                args, f'umount {flag}{q(slot["guest_dst"])}', use_sudo=True
            ),
        )
        show_if_any(
            f'findmnt guest dst {slot["name"]} (after)',
            guest_findmnt(args, slot['guest_dst']),
        )


def sync_probe_for_slot(
    args: argparse.Namespace, slot: dict[str, str], iteration: int
) -> dict[str, Any]:
    ...
    guest_seen = guest_read_probe(
        args, slot['guest_dst'] + '/host_to_guest.txt'
    )
...
def iteration_probe(
    args: argparse.Namespace, state: dict[str, Any], iteration: int
) -> dict[str, Any]:
    slot_results = [
        sync_probe_for_slot(args, slot, iteration) for slot in state['slots']
    ]
    active_snapshot = relevant_holder_snapshot(
        args, state, label=f'iter-{iteration}-active'
    )
    return {
        'iteration': iteration,
...
def run_policy_measurement(
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        result['active_delta_from_baseline'] = snapshot_delta(
            result['active_snapshot'], baseline
        )
        result['post_detach_snapshot'] = post_detach_snapshot
        result['post_detach_delta_from_baseline'] = snapshot_delta(
            post_detach_snapshot, baseline
        )
...
def compare_policies(args: argparse.Namespace) -> int:
    ...
    for policy in policies:
        ...
        phase_summaries[policy] = run_policy_measurement(
            phase_args, phase_state
        )
...
def cleanup(args: argparse.Namespace, state: dict[str, Any]) -> int:
    ...
    for slot in state['slots']:
        show_if_any(
            f'rmdir guest dst {slot["name"]}',
            guest_run(args, f'rmdir {q(slot["guest_dst"])}', use_sudo=True),
        )
        show_if_any(
            f'rmdir host stage {slot["name"]}',
            host_run(args, ['rmdir', slot['host_stage']]),
        )
        show_if_any(
            f'rm -rf host src {slot["name"]}',
            host_run(args, ['rm', '-rf', slot['host_src']]),
        )

    show_if_any(
        'rmdir guest dst root',
        guest_run(
            args, f'rmdir {q(state["paths"]["guest_dst_root"])}', use_sudo=True
        ),
    )
    show_if_any(
        'rmdir host stage root',
        host_run(args, ['rmdir', state['paths']['host_stage_root']]),
    )
    show_if_any(
        'rm -rf host scratch root',
        host_run(args, ['rm', '-rf', state['paths']['host_scratch_root']]),
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description='Standalone FD-growth MWE for shared-root attachment designs'
    )
    ...
    def add_common(sp: argparse.ArgumentParser) -> None:
        sp.add_argument(
            '--ssh-target',
            required=True,
            help='for example aivm-2404 or user@ip',
        )
        ...
        sp.add_argument(
            '--guest-shared-base', default=DEFAULT_GUEST_SHARED_BASE
        )
        sp.add_argument('--host-export-dir', default=None)
        sp.add_argument(
            '--shared-root-policy',
            choices=['managed', 'reuse'],
            default='managed',
        )
        sp.add_argument('--virtiofs-tag', default=None)
        sp.add_argument(
            '--fail-dirty-preflight',
            dest='fail_dirty_preflight',
            action='store_true',
            default=True,
        )
        sp.add_argument(
        ...
        sp.add_argument(
            '--no-host-sudo', dest='host_sudo', action='store_false'
        )
        ...
        sp.add_argument(
            '--force-destroy-on-shutdown-timeout',
            dest='force_destroy_on_shutdown_timeout',
            action='store_true',
            default=True,
        )
        sp.add_argument(
            '--no-force-destroy-on-shutdown-timeout',
            dest='force_destroy_on_shutdown_timeout',
            action='store_false',
        )
    ...
    loop_cmd.add_argument(
        '--host-policy',
        choices=['stable-host', 'churn-host'],
        default='stable-host',
    )
    ...
    compare_cmd.add_argument(
        '--restart-vm-between-policies', action='store_true', default=True
    )
    compare_cmd.add_argument(
        '--no-restart-vm-between-policies',
        dest='restart_vm_between_policies',
        action='store_false',
    )
