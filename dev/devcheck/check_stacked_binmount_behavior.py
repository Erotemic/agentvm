...
def alias_seen_in_lsof_plus_d(alias: Path, pids: list[int]) -> tuple[bool, str]:
    ...
    res = capture_cmd(["bash", "-lc", f"sudo lsof +D {alias!s} | egrep '({expr})|COMMAND' || true"])
...
def concludes_expected_behavior(
    *,
    ...
    src_before_ok = before_src_report["readlink"] == str(src) and before_src_report["realpath"] == str(src)
    src_after_ok = after_src_report["readlink"] == str(src) and after_src_report["realpath"] == str(src)
    alias_proc_ok = alias_report["readlink"] == str(alias) and alias_report["realpath"] == str(alias)
    stacked_ok = layer_count >= 2

    if stacked_ok:
        evidence.append(f"Observed stacked bind layers at alias target: {layer_count}")
    if src_before_ok:
        evidence.append("Process started in SRC before stacking still reports cwd via SRC")
    if src_after_ok:
        evidence.append("Process started in SRC after stacking still reports cwd via SRC")
    if alias_proc_ok:
        evidence.append("Process started in ALIAS reports cwd via ALIAS")
    if lsof_plus_d_hits:
        evidence.append("lsof +D ALIAS surfaces the tracked processes through the alias tree")

    confirmed = stacked_ok and src_before_ok and src_after_ok and alias_proc_ok and lsof_plus_d_hits
...
def main() -> int:
    ...
    try:
        print("\nSTEP 1: no mount yet")
        run(["bash", "-lc", f"cd {src!s} && echo 'shell in SRC before mount:' && pwd && pwd -P"])
        ...
        time.sleep(0.2)
        src_before_report = show_proc("started in SRC after 1 bind", p_src_1.pid)
        ...
        time.sleep(0.2)
        src_after_report = show_proc("started in SRC after stacked binds", p_src_2.pid)
        ...
        lsof_hits, lsof_text = alias_seen_in_lsof_plus_d(alias, [p_src_1.pid, p_alias_1.pid, p_src_2.pid])
        if lsof_text:
            print(lsof_text)
        ...
        if KEEP:
            ...
            print(f"  SRC={src}")
            print(f"  ALIAS={alias}")
            for name, proc in [("p_src_1", p_src_1), ("p_alias_1", p_alias_1), ("p_src_2", p_src_2)]:
                if proc is not None:
