...
def guest_discovery_report(args: argparse.Namespace) -> dict:
    ...
    res_json = guest_run(
        args,
        'command findmnt --json --list -t virtiofs -o TARGET,SOURCE,FSTYPE,OPTIONS',
    )
    report['findmnt_json'] = {
        'ok': res_json.ok,
        ...
        try:
            ...
            if not s.startswith('{'):
                report['errors'].append(
                    f'findmnt_json_not_json: first_bytes={s[:80]!r}'
                )
            else:
    ...
    res_pairs = guest_run(
        args,
        'command findmnt --pairs --list -t virtiofs -o TARGET,SOURCE,FSTYPE,OPTIONS',
    )
    report['findmnt_pairs'] = {
        'ok': res_pairs.ok,
        'stdout': res_pairs.stdout,
        'stderr': res_pairs.stderr,
    }
    if res_pairs.ok and res_pairs.stdout.strip():
        try:
            first = next(
                (
                    ln.strip()
                    for ln in res_pairs.stdout.splitlines()
                    if ln.strip()
                ),
                '',
            )
            if '=' not in first:
                report['errors'].append(
                    f'findmnt_pairs_not_pairs: first_line={first[:120]!r}'
                )
            else:
    ...
    res_mountinfo = guest_run(
        args, "command awk '$0 ~ / - virtiofs / { print }' /proc/self/mountinfo"
    )
    report['mountinfo'] = {
        'ok': res_mountinfo.ok,
        'stdout': res_mountinfo.stdout,
        'stderr': res_mountinfo.stderr,
    }
    ...
    res_mount = guest_run(
        args,
        "command mount | command grep ' type virtiofs ' || true",
        use_sudo=True,
    )
    report['mount_cmd'] = {
        'ok': res_mount.ok,
        'stdout': res_mount.stdout,
        'stderr': res_mount.stderr,
    }
...
def discover_guest_shared_base(
    args: argparse.Namespace,
) -> tuple[str, str, dict]:
    ...
    if args.guest_shared_base:
        ...
        res = guest_run(
            args,
            f'findmnt -P -T {q(args.guest_shared_base)} -o TARGET,SOURCE,FSTYPE,OPTIONS',
        )
        if res.ok and res.stdout.strip():
            pairs = _parse_findmnt_pairs(res.stdout)
            good_pairs = [
                p for p in pairs if p.get('target') and p.get('source')
            ]
            if good_pairs:
                item = good_pairs[0]
                return str(item['target']), str(item['source']), report
        raise RuntimeError(
            f'could not inspect explicit guest shared base {args.guest_shared_base!r}'
        )
...
def discover_host_export_dir(
    args: argparse.Namespace, vm_name: str, source_tag: str
) -> str:
    if args.host_export_dir:
        return args.host_export_dir

    res = host_run(
        args, ['virsh', '-c', 'qemu:///system', 'dumpxml', vm_name], check=True
    )
...
def derive_state(
    ...
    guest_dst = args.guest_dst or str(
        Path(args.guest_scratch_root) / runid / 'dst'
    )
    ...
    host = {
        'scratch_root': host_scratch_root,
        'src': host_src,
        'stage': host_stage,
    }
...
def host_stat(args: argparse.Namespace, path: str) -> CmdResult:
    return host_run(
        ...
        [
            'stat',
            '-Lc',
            'mode=%A uid=%u gid=%g dev=%d ino=%i type=%F path=%n',
            path,
        ],
    )
...
def guest_findmnt(args: argparse.Namespace, path: str) -> CmdResult:
    return guest_run(
        args,
        f'findmnt -T {q(path)} -o TARGET,SOURCE,FSTYPE,OPTIONS,PROPAGATION',
    )
...
def guest_stat(args: argparse.Namespace, path: str) -> CmdResult:
    return guest_run(
        args,
        f'stat -Lc {q("mode=%A uid=%u gid=%g dev=%d ino=%i type=%F path=%n")} {q(path)}',
    )
...
def host_git_probe(args: argparse.Namespace, path: str) -> list[CmdResult]:
    out = [
        host_run(
            args, ['git', '-C', path, 'rev-parse', '--is-inside-work-tree']
        )
    ]
    if out[-1].ok and out[-1].stdout.strip() == 'true':
        out.append(
            host_run(args, ['git', '-C', path, 'status', '--short', '--branch'])
        )
    return out
...
def guest_git_probe(args: argparse.Namespace, path: str) -> list[CmdResult]:
    out = [
        guest_run(
            args,
            f'git -C {q(path)} rev-parse --is-inside-work-tree',
            use_sudo=True,
        )
    ]
    if out[-1].ok and out[-1].stdout.strip() == 'true':
        out.append(
            guest_run(
                args, f'git -C {q(path)} status --short --branch', use_sudo=True
            )
        )
    return out
...
def guest_repo_probe(args: argparse.Namespace, path: str) -> list[CmdResult]:
    out = [
        guest_run(
            args,
            f"bash -lc 'test -d {q(str(Path(path) / '.git'))} && echo yes || echo no'",
            use_sudo=True,
        ),
...
def prepare_host_source(
    args: argparse.Namespace, env: dict[str, str], host: dict[str, str]
) -> None:
    banner('HOST PREPARE')
    show(
        'mkdir scratch root',
        host_run(args, ['mkdir', '-p', host['scratch_root']]),
    )
    show('mkdir host src', host_run(args, ['mkdir', '-p', host['src']]))
    show(
        'mkdir host stage parent',
        host_run(args, ['mkdir', '-p', str(Path(host['stage']).parent)]),
    )
    ...
    if args.init_git:
        show(
            'git init', host_run(args, ['git', '-C', host['src'], 'init', '-q'])
        )
        show(
            'git config user.name',
            host_run(
                args, ['git', '-C', host['src'], 'config', 'user.name', 'MWE']
            ),
        )
        show(
        ...
        show(
            'git commit',
            host_run(
                args, ['git', '-C', host['src'], 'commit', '-qm', 'initial mwe']
            ),
        )
...
def attach(
    ...
    if host_is_exact_mountpoint(args, host['stage']):
        ...
        show(
            'mount --bind host_src -> host_stage',
            host_run(args, ['mount', '--bind', host['src'], host['stage']]),
        )
    ...
    if guest_is_exact_mountpoint(args, guest['dst']):
        show(
            'findmnt guest dst (already mounted)',
            guest_findmnt(args, guest['dst']),
        )
    else:
        show(
            'mount --bind guest_stage -> guest_dst',
            guest_run(
                args,
                f'mount --bind {q(guest["stage"])} {q(guest["dst"])}',
                use_sudo=True,
            ),
        )
        if args.access == 'ro':
            show(
                'remount guest dst ro',
                guest_run(
                    args,
                    f'mount -o remount,bind,ro {q(guest["dst"])}',
                    use_sudo=True,
                ),
            )
...
def probe(
    args: argparse.Namespace, host: dict[str, str], guest: dict[str, str]
) -> None:
    banner('HOST PROBE')
...
def detach_guest(args: argparse.Namespace, guest: dict[str, str]) -> None:
    ...
    show(
        'umount guest dst',
        guest_run(args, f'umount {flag}{q(guest["dst"])}', use_sudo=True),
    )
...
def detach_host(args: argparse.Namespace, host: dict[str, str]) -> None:
    banner('DETACH HOST')
    show_if_any(
        'findmnt host stage (before)', host_findmnt(args, host['stage'])
    )
...
def reconcile_detach(
    ...
    if guest_is_exact_mountpoint(args, guest['dst']):
        flag = '-l ' if args.lazy_guest else ''
        show(
            'retry umount guest dst',
            guest_run(args, f'umount {flag}{q(guest["dst"])}', use_sudo=True),
        )
        show_if_any(
            'findmnt guest dst (reconcile)', guest_findmnt(args, guest['dst'])
        )
    ...
    if host_is_exact_mountpoint(args, host['stage']):
        ...
        for attempt in range(1, retries + 1):
            ...
            argv.append(host['stage'])
            show(
                f'retry umount host stage attempt={attempt}/{retries}',
                host_run(args, argv),
            )
            show_if_any(
                f'findmnt host stage (reconcile attempt={attempt})',
                host_findmnt(args, host['stage']),
            )
...
def cleanup(
    ...
    if guest_is_exact_mountpoint(args, guest['dst']):
        if args.keep_on_cleanup_fail:
            banner('KEEP')
            print(
                f'keeping scratch paths because guest dst is still mounted: {guest["dst"]}'
            )
            return
        raise RuntimeError(
            f'refusing cleanup: guest dst still mounted: {guest["dst"]}'
        )
    if host_is_exact_mountpoint(args, host['stage']):
        if args.cleanup_lazy_host_on_fail:
            banner('CLEANUP HOST LAZY UMOUNT')
            show(
                'lazy umount host stage for cleanup',
                host_run(args, ['umount', '-l', host['stage']]),
            )
            show_if_any(
                'findmnt host stage (after lazy cleanup)',
                host_findmnt(args, host['stage']),
            )
        if host_is_exact_mountpoint(args, host['stage']):
            if args.keep_on_cleanup_fail:
                banner('KEEP')
                print(
                    f'keeping scratch paths because host stage is still mounted: {host["stage"]}'
                )
                return
            raise RuntimeError(
                f'refusing cleanup: host stage still mounted: {host["stage"]}'
            )
    banner('CLEANUP')
    ...
    show_if_any(
        ...
        host_run(args, ['rm', '-rf', host['scratch_root']]),
    )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description='Standalone non-invasive mount/unmount MWE'
    )
    sub = p.add_subparsers(dest='cmd', required=True)

    def add_common(sp: argparse.ArgumentParser) -> None:
        sp.add_argument(
            '--ssh-target', required=True, help='for example agent@10.77.0.195'
        )
        ...
        sp.add_argument(
            '--init-git', dest='init_git', action='store_true', default=True
        )
    ...
    detach_cmd.add_argument(
        '--order',
        choices=['guest-first', 'host-first', 'guest-only', 'host-only'],
        default='guest-first',
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if report and vm_name and args.host_export_dir is None:
            ...
            real_sources = [
                c for c in candidates if c.get('source') not in (None, 'none')
            ]
            if real_sources:
                try:
                    discover_host_export_dir(
                        args, vm_name, str(real_sources[0]['source'])
                    )
    ...
    if args.cmd == 'detach':
        ...
        banner('DETACH ORDER')
        print(
            f'order={args.order} lazy_host={args.lazy_host} lazy_guest={args.lazy_guest}'
        )
