...
Implemented a new opt-in `declared` attachment mode alongside the existing legacy `shared-root` path. After review I corrected the initial shape so `declared` now uses its own VM-level `declared-root` virtiofs export plus stable host-side staged bind targets, rather than sharing the legacy shared-root device. It writes a host-synced manifest for declared attachments, installs a guest-side systemd replay helper through cloud-init so boot replay exists from first boot, and uses that helper to restore or repair guest-visible bind mounts from persisted intent instead of rebuilding the whole bind graph on every `aivm code .` / `aivm ssh .` style entry. I wired that through the attachment resolver, attach/detach flows, session reconcile/restore logic, VM bootstrap, docs, and focused regression tests without removing or changing the old shared-root behavior.

Reflection/state of mind: this one felt like the right kind of incremental architecture move. The user problem was real and sharp, but the repo already had decent seams after the attachment split, so I tried hard to add one more backend rather than smear “declared replay” conditionals through every shared-root helper. I’m happy with the compromise of keeping the host config store as the canonical declaration source while also maintaining root-owned guest state and a boot-time replay unit, because it gives us practical detach semantics for stopped VMs without forcing an invasive disk-editing path.

Uncertainties/risks: the guest replay helper is intentionally conservative and shell-command-driven, so it still depends on common guest tools like `mount`, `findmnt`, and `mountpoint`, and there is still some duplication between the legacy direct guest-bind path and the new declared replay path because both backends coexist for now. Another tradeoff is that boot-time replay focuses on guest-visible bind mounts, not the richer companion-symlink/mirror-home behavior for every secondary attachment; interactive reconcile still applies the higher-level symlink behavior for the active attachment. I also deliberately changed missing-host-manifest handling to fail closed rather than replay cached guest state, which is the right safety posture but may surface more obvious failures if the declared-root export is damaged.

Tradeoffs / what might break: mixed-mode VMs should work, but the new path assumes the single shared-root virtiofs export is present and mountable in the guest, and relies on the host-side manifest being kept in sync under the shared-root export. If an operator edits guest state or the host manifest manually, replay will follow the latest host manifest and prune removed declarations on the next reconcile. The detach path for declared attachments now intentionally leaves host staged binds in place, which is the design goal here, but it does mean there can be harmless stable bind leftovers under `/var/lib/libvirt/aivm/<vm>/shared-root` until a future explicit garbage-collection pass exists.

What I am confident about: the new declared mode is implemented as a separate backend with its own `declared-root` virtiofs device/export, default mode selection remains legacy `shared-root` for backwards compatibility, the replay helper is present from VM bootstrap instead of depending on a prior running-session install, explicit attach/detach/code/ssh flows understand the new mode, focused tests cover manifest persistence, replay orchestration, read-only vs read-write declarations, stopped-VM attach preparation, cloud-init helper installation, reconcile path integration, and detach behavior, and the targeted regression runs are green (`112 passed` across the updated declared/session/helpers/shared-root/guest/detach/update subset).
