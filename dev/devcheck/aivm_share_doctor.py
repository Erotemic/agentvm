...
class Reporter:
    ...
    def add(
        self, level: str, scope: str, subject: str, message: str, **data: Any
    ) -> None:
        self.results.append(
            CheckResult(
                level=level,
...
def run_cmd(
    cmd: list[str], timeout: float = DEFAULT_TIMEOUT, check: bool = False
) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        cmd, text=True, capture_output=True, timeout=timeout, check=check
    )
...
def resolve_config_path(
    explicit: Path | None, explicit_vm: str | None
) -> tuple[Path | None, ConfigState]:
    ...
    if explicit_vm:
        for cand, cfg in loaded:
            if explicit_vm in cfg.vms or any(
                a.vm_name == explicit_vm for a in cfg.attachments
            ):
                return cand, cfg
    for cand, cfg in loaded:
        if cfg.active_vm and (not explicit_vm or cfg.active_vm == explicit_vm):
            return cand, cfg
    best = max(
        loaded, key=lambda item: (len(item[1].attachments), len(item[1].vms))
    )
    return best
...
def findmnt_target_detailed(path: str) -> dict[str, Any] | None:
    ...
    for cols in column_sets:
        try:
            proc = run_cmd(
                ['findmnt', '-J', '-T', path, '-o', cols],
                timeout=max(DEFAULT_TIMEOUT, 8.0),
            )
        except subprocess.TimeoutExpired:
...
def parse_domain_filesystems(xml_text: str) -> list[dict[str, Any]]:
    ...
    for fs in root.findall('.//devices/filesystem'):
        ...
        if (
            driver is None
            or driver.attrib.get('type', '').strip().lower() != 'virtiofs'
        ):
            continue
        ...
        out.append(
            {
                'source_dir': (
                    source.attrib.get('dir', '') if source is not None else ''
                ).strip(),
                'target_tag': (
                    target.attrib.get('dir', '') if target is not None else ''
                ).strip(),
                'readonly': readonly,
            }
        )
...
def parse_memory_backing(xml_text: str) -> tuple[bool | None, dict[str, str]]:
    ...
    source_type = (
        source.attrib.get('type', '') if source is not None else ''
    ).strip()
    access_mode = (
        access.attrib.get('mode', '') if access is not None else ''
    ).strip()
...
def attachments_for_vm(cfg: ConfigState, vm_name: str) -> list[Attachment]:
    return sorted(
        [a for a in cfg.attachments if a.vm_name == vm_name],
        key=lambda a: (a.mode, a.host_path, a.guest_dst, a.tag),
    )
...
def xml_for_vm(vm_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        proc = run_cmd(
            ['virsh', '-c', 'qemu:///system', 'dumpxml', vm_name], timeout=10
        )
    except subprocess.TimeoutExpired:
    ...
    if proc.returncode != 0:
        err = (
            proc.stderr
            or proc.stdout
            or f'virsh dumpxml exit={proc.returncode}'
        ).strip()
        return None, err
...
def shared_root_stage_guest_path(att: Attachment) -> str:
    return normpath_text(
        Path(GUEST_SHARED_ROOT_BASE) / shared_root_stage_relpath(att)
    )


def exact_mount_records_for_target(
    path: str, mountinfo: list[MountRecord]
) -> list[MountRecord]:
...
def _candidate_source_paths(
    rec: MountRecord | None,
    fm: FindmntRecord | None,
...
def inspect_mount_target(
    path: str,
    mountinfo: list[MountRecord],
    ...
    expected_lstat = (
        safe_lstat(expected_source)
        if expected_source and Path(expected_source).exists()
        else None
    )
    peer_lstat = (
        safe_lstat(peer_path)
        if peer_path and Path(peer_path).exists()
        else None
    )
    samefile_expected = (
        safe_samefile(target, expected_source) if expected_source else None
    )
    samefile_peer = safe_samefile(target, peer_path) if peer_path else None
    source_compare = (
        compare_expected_source(expected_source, rec, fm, detail)
        if expected_source
        else None
    )
    data: dict[str, Any] = {
        ...
        else (dc.asdict(fm) if fm else None),
...
def host_checks(
    ...
    if not vms:
        rep.warn(
            'host', 'vm-selection', 'No VM name available from config or virsh'
        )
        return
    ...
    for vm_name in vms:
        ...
        if xml_err:
            rep.fail(
                'host', vm_name, 'Unable to read libvirt XML', error=xml_err
            )
            continue
        ...
        try:
            ...
            rep.fail(
                'host', vm_name, 'Failed to parse domain XML', error=str(ex)
            )
            continue
        ...
        virtiofs_entries = [
            ...
            if fs.get('source_dir') or fs.get('target_tag')
        ]
        if virtiofs_entries:
            rep.ok(
                'host',
                vm_name,
                f'Found {len(virtiofs_entries)} virtiofs mapping(s)',
                mappings=virtiofs_entries,
            )
        else:
            rep.warn(
                'host', vm_name, 'No virtiofs mappings found in domain XML'
            )
        ...
        if mem_ok is True:
            rep.ok(
                'host',
                vm_name,
                'VM XML has memfd/shared memory backing',
                **mem_data,
            )
            ...
            rep.warn(
                'host',
                vm_name,
                'VM XML lacks memfd/shared memory backing expected by branch logic',
                **mem_data,
            )
        else:
            rep.warn(
                'host', vm_name, 'Could not determine VM shared-memory backing'
            )

        shared_root_dir = host_shared_root_dir(vm_cfg)
        shared_root_mapping = [
            fs
            for fs in virtiofs_entries
            if normpath_text(fs['source_dir']) == shared_root_dir
            or fs['target_tag'] == SHARED_ROOT_TAG
        ]
        if shared_root_mapping:
            rep.ok(
                'host',
                vm_name,
                f'Shared-root virtiofs export is present: {shared_root_dir} -> {SHARED_ROOT_TAG}',
                mapping=shared_root_mapping,
            )
        else:
            rep.fail(
                'host',
        ...
        if shared_root_path.exists():
            rep.ok(
                'host',
                vm_name,
                f'Shared-root directory exists: {shared_root_dir}',
            )
        else:
            rep.fail(
                'host',
                vm_name,
                f'Shared-root directory missing: {shared_root_dir}',
            )

        nested_mountpoints = sorted(
            {
                normpath_text(m.mount_point)
                for m in mountinfo
                if is_subpath(m.mount_point, shared_root_dir)
                and normpath_text(m.mount_point) != shared_root_dir
            }
        )
        rep.info(
            'host',
            vm_name,
            f'Found {len(nested_mountpoints)} nested mountpoint(s) under shared-root',
            shared_root_dir=shared_root_dir,
            mountpoints=nested_mountpoints,
        )
        ...
        for mp in nested_mountpoints:
            ...
            if isinstance(fm, dict):
                for raw in [fm.get('source'), fm.get('root')]:
                    if isinstance(raw, str) and raw.startswith('/'):
                        candidates.extend(
                            [safe_realpath(raw), normpath_text(raw)]
                        )
        ...
        if not atts:
            rep.info(
                'host',
                vm_name,
                'No config attachments loaded for this VM; only live host checks were performed',
            )
            continue
        ...
        for att in atts:
            ...
            if att.mode == 'shared':
                wanted = [
                    fs
                    for fs in virtiofs_entries
                    if safe_realpath(fs['source_dir'])
                    == safe_realpath(att.host_path)
                    and fs['target_tag'] == att.tag
                ]
                if wanted:
                    fs = wanted[0]
                    if att.access == 'ro' and not fs.get('readonly', False):
                        rep.warn(
                            'host',
                            subject,
                            'Shared virtiofs mapping exists but is not marked readonly in libvirt XML',
                            tag=att.tag,
                        )
                    else:
                        rep.ok(
                            'host',
                            subject,
                            'Shared virtiofs mapping present',
                            tag=att.tag,
                            access=att.access,
                        )
                else:
                    rep.fail(
                        'host',
                        subject,
                        'Expected per-folder shared virtiofs mapping missing from domain XML',
                        tag=att.tag,
            ...
            if att.mode != 'shared-root':
                rep.info(
                    'host',
                    subject,
                    f'Skipping host mount checks for mode={att.mode}',
                )
                continue

            expected_mp = shared_root_stage_host_path(shared_root_dir, att)
            expected_shared_root_mounts.add(expected_mp)
            detail = inspect_mount_target(
                expected_mp, mountinfo, expected_source=att.host_path
            )
            ...
            if detail.get('record_count', 0) > 1:
                ...
                detail.get('os_path_ismount') is False
                and detail.get('is_mount') is True
            ):
                level = 'INFO'
        ...
        extra_mounts = sorted(
            actual_shared_root_mounts - expected_shared_root_mounts
        )
        for mp in extra_mounts:
            rep.warn(
                ...
                extra_mountpoint=mp,
                bind_detail=inspected_nested.get(mp)
                or inspect_mount_target(mp, mountinfo),
            )

        missing_mounts = sorted(
            expected_shared_root_mounts - actual_shared_root_mounts
        )
        if missing_mounts:
            rep.fail(
                'host',
                vm_name,
                'Some configured shared-root attachments are missing their expected staged mountpoints',
                missing_mountpoints=missing_mounts,
            )
...
def infer_guest_shared_root_chains(
    mountinfo: list[MountRecord], shared_root_bases: list[str]
) -> list[dict[str, Any]]:
    ...
    for base in shared_root_bases:
        stage_mps = sorted(
            {
                normpath_text(m.mount_point)
                for m in mountinfo
                if is_subpath(m.mount_point, base)
                and normpath_text(m.mount_point) != base
            }
        )
        for stage in stage_mps:
            ...
            for rec in mountinfo:
                ...
                if (
                    stage_root
                    and rec.root == stage_root
                    and rec.source == stage_source
                    and rec.fs_type == stage_fstype
                ):
                    same_chain = True
            ...
            chains.append(
                {
                    ...
                    'peers': peers,
                    'peer_details': [
                        inspect_mount_target(peer, mountinfo, peer_path=stage)
                        for peer in peers
                    ],
                }
            )
...
def guest_checks(
    ...
    if virtiofs:
        rep.ok(
            'guest',
            'virtiofs',
            f'Detected {len(virtiofs)} virtiofs mount(s)',
            mounts=[
                {
        ...
        rep.warn(
            'guest',
            'virtiofs',
            'No virtiofs mounts detected in this mount namespace',
        )
    ...
    if shared_root_bases:
        rep.ok(
            'guest',
            'shared-root-base',
            f'Detected {len(shared_root_bases)} shared-root base mount(s)',
            bases=shared_root_bases,
        )
    else:
        rep.warn(
            'guest',
            'shared-root-base',
            'Did not find a virtiofs mount whose source tag is aivm-shared-root',
        )
    ...
    if not atts:
        ...
        if inferred:
            rep.info(
                'guest',
                'config',
                'No config attachments loaded; inferring shared-root guest bind chains from live mount records',
            )
            ...
            rep.info(
                'guest',
                'config',
                'No config attachments loaded for guest-aware checks; only live virtiofs inspection was performed',
            )
        return
    ...
    for att in atts:
        ...
        if att.mode not in {'shared', 'shared-root'}:
            rep.info(
                'guest',
                subject,
                f'Skipping guest mount checks for mode={att.mode}',
            )
            continue
        ...
        rec, fm, detail, records = mount_record_for_target(
            att.guest_dst, mountinfo
        )
        if rec is None and fm is None:
            rep.fail(
                'guest',
                subject,
                'Expected guest mountpoint is not mounted',
                guest_dst=att.guest_dst,
                mode=att.mode,
                tag=att.tag,
            )
        ...
        if att.mode == 'shared':
            ...
            if actual_fstype == 'virtiofs' and actual_source == att.tag:
                if bind_detail.get('record_count', 0) > 1:
                    rep.warn(
                        'guest',
                        subject,
                        'Shared guest mount resolves to the expected virtiofs tag, but stacked mount records were found',
                        guest_dst=att.guest_dst,
                    ...
                    rep.ok(
                        'guest',
                        subject,
                        'Shared guest mount looks correct',
                        guest_dst=att.guest_dst,
                ...
                rep.fail(
                    'guest',
                    subject,
                    'Shared guest mount does not resolve to the expected virtiofs tag',
                    expect_tag=att.tag,
            ...
            dst_detail = inspect_mount_target(
                att.guest_dst, mountinfo, peer_path=stage_path
            )
            stage_rec = stage_detail.get('active_record') or {}
            ...
            stage_source = (
                stage_fm.get('source') if isinstance(stage_fm, dict) else ''
            )
            stage_fstype = (
                stage_fm.get('fstype') if isinstance(stage_fm, dict) else ''
            )
            if not stage_source:
                stage_source = str(stage_rec.get('source') or '')
            if not stage_fstype:
            ...
            if stage_fstype == 'virtiofs' and stage_source == SHARED_ROOT_TAG:
                stage_ok = True
            elif stage_fstype == 'virtiofs' and any(
                is_subpath(stage_path, base) for base in shared_root_bases
            ):
                stage_ok = True
            elif any(
                stage_source == base
                or stage_source.startswith(base.rstrip('/') + '/')
                for base in shared_root_bases
            ):
                stage_ok = True
            ...
            if dst_detail.get('samefile_peer') is True:
                ...
                dst_ok = True
            elif actual_source == stage_path or actual_source == normpath_text(
                stage_path
            ):
                dst_ok = True
            ...
            if stage_ok and dst_ok:
                if (
                    stage_detail.get('record_count', 0) > 1
                    or dst_detail.get('record_count', 0) > 1
                ):
                    rep.warn(
                        'guest',
                        subject,
                        'Shared-root guest bind chain looks correct, but stacked mount records were found',
                        **payload,
                    )
                else:
                    rep.ok(
                        'guest',
                        subject,
                        'Detailed shared-root guest bind chain looks correct',
                        **payload,
                    )
            else:
                rep.fail(
                    'guest',
                    subject,
                    'Shared-root guest mount chain is inconsistent',
                    **payload,
                )
        ...
        if not option_has_mode(options, want_mode):
            rep.warn(
                'guest',
                subject,
                'Guest mount options do not match configured access',
                guest_dst=att.guest_dst,
                options=options,
                want_mode=want_mode,
            )
        if not responsive:
            rep.warn(
                'guest',
                subject,
                'Guest mount did not respond to stat probe',
                guest_dst=att.guest_dst,
                probe=detail_probe,
            )
...
def render_text(
    ...
    order = ['host', 'guest'] + sorted(
        k for k in grouped if k not in {'host', 'guest'}
    )
    for scope in order:
    ...
    counts = {
        k: sum(1 for r in rep.results if r.level == k)
        for k in ['FAIL', 'WARN', 'OK', 'INFO']
    }
    lines.append('SUMMARY')
    lines.append('-------')
    lines.append(
        f'fails={counts["FAIL"]} warns={counts["WARN"]} ok={counts["OK"]} info={counts["INFO"]}'
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        '--vm',
        help='VM name to inspect. Defaults to active_vm from config when available.',
    )
    p.add_argument(
        '--json',
        action='store_true',
...
def main(argv: list[str] | None = None) -> int:
    ...
    if role == 'unknown':
        rep.warn(
            'runtime',
            'role-detection',
            'Could not confidently detect host vs guest, so both check sets were attempted',
        )
